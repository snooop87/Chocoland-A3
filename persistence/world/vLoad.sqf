		...
		for "_i" from 1 to _objectsCount do
		{
			...
			if (!isNil "_class" && {!isNil "_pos"} && {_maxLifetime <= 0 || {_hoursAlive < _maxLifetime}}) then 
			{
				...
				if (_allowed) then
				{
					...
					//_obj lock 2; // loads the saved vehicle locked
					//_obj setVariable ["locked", 2, true];  // loads the saved vehicle locked
					//_obj setVariable ["objectLocked", true, true]; // loads the saved vehicle locked, this one is probably never needed for vehicles
					//_obj setVariable ["R3F_LOG_disabled",true,true]; //loads the vehicle with towing and lifting disabled, used together with locking, where the locking scripts turn on or of towing
					_obj disableTIEquipment true; // disables thermal equipment on loaded vehicles, comment out if you want thermals	
					//_obj setVariable ["CanSell", 1, true]; // allows for selling vehicles in SNAFU mission
					//_obj removeMagazinesTurret ["6Rnd_155mm_Mo_AT_mine", [0]]; //remove AT mine ammo, artilley is in SNAFU shops but i dont like this ammo
					//_obj removeMagazinesTurret ["6Rnd_155mm_Mo_mine", [0]]; //remove AT mine ammo, artilley is in SNAFU shops but i dont like this ammo
